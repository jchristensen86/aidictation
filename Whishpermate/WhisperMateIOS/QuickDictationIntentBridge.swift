...
final class QuickDictationIntentBridge {
    ...
    private func cancel(request: Int) {
        ...
        resolve(.failure(CancellationError()), cancelRecording: true)
    }
    ...
    private func scheduleTimeout(
        ...
        Task { @MainActor in
            ...
            self.resolve(.failure(error), cancelRecording: true)
        }
    }

    private func resolve(_ result: Result<String, Error>, cancelRecording: Bool = false) {
        let cancel = cancelRecording ? cancellationHandler : nil
