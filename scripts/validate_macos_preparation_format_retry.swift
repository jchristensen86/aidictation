...
private struct SourceChangePinningValidator {
    ...
    private static func validateSourceContract() throws {
        ...
        // The chosen microphone is applied as the system default before
        // capture. AVAudioEngine must then keep ownership of its HAL unit and
        // private aggregate device. Rebinding that unit to raw hardware can
        // make the tap retain the aggregate's sample rate while the hardware
        // switches rates, which makes engine.start() fail.
        try require(
            !recorderSource.contains("kAudioOutputUnitProperty_CurrentDevice"),
            "AudioRecorder must not rebind AVAudioEngine's HAL unit"
        )
        try require(
            !recorderSource.contains("bindInputDevice"),
            "AudioRecorder must let AVAudioEngine retain its private aggregate device"
        )
        try require(
        ...
        try require(
            prepareMethod.contains("installTap(onBus: bus, bufferSize: 2048, format: nil)"),
            "prepareCapture must install its tap using AVAudioEngine's current input format"
        )
        try require(
